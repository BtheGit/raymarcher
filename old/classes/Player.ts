...
class Player {
  constructor(game, pos, dir, plane) {
    this.game = game;
    this.publishPlayerMove = publishPlayerMove(this.game.eventHandler.publish)(
      this
    );
  ...
  castRay(cameraX, castDistance = Infinity) {
    ...
    let sideDistX =
      (rayDir.x < 0
        ? this.pos.x - activeCell.x
        : 1 + activeCell.x - this.pos.x) * distanceDelta.x;
    let sideDistY =
      (rayDir.y < 0
        ? this.pos.y - activeCell.y
        : 1 + activeCell.y - this.pos.y) * distanceDelta.y;
  ...
  checkSpritesForCollisions(x, y) {
    const collisionDetected = this.game.sprites.some((sprite) => {
      ...
      const distance = Math.sqrt(
        Math.pow(x - spriteX, 2) + Math.pow(y - spriteY, 2)
      );
  ...
  moveForward(modifier = 1) {
    ...
    if (checkIsFloor(nextCellX)) {
      const collisionDetected = this.checkSpritesForCollisions(
        newPosX,
        this.pos.y
      );
  ...
  rotate(rotation) {
    ...
    // speed changes.
    const newDirX =
      this.dir.x * Math.cos(this.rotationSpeed * rotation) -
      this.dir.y * Math.sin(this.rotationSpeed * rotation);
    const newDirY =
      this.dir.x * Math.sin(this.rotationSpeed * rotation) +
      this.dir.y * Math.cos(this.rotationSpeed * rotation);
    const newPlaneX =
      this.plane.x * Math.cos(this.rotationSpeed * rotation) -
      this.plane.y * Math.sin(this.rotationSpeed * rotation);
    const newPlaneY =
      this.plane.x * Math.sin(this.rotationSpeed * rotation) +
      this.plane.y * Math.cos(this.rotationSpeed * rotation);
  ...
  trigger() {
    ...
    if (sprites.length > 0) {
      ...
      const closestUnobstructedSpriteWithTrigger = sprites.reduce(
        (acc, sprite) => {
          const spriteDistance =
            Math.pow(this.pos.x - sprite.pos.x, 2) +
            Math.pow(this.pos.y - sprite.pos.y, 2);
          if (spriteDistance < wallDistance) {
            ...
            const transformX =
              this.inverseDeterminate *
              (this.dir.y * spriteX_relativeToPlayer -
                this.dir.x * spriteY_relativeToPlayer);
            const transformY = Math.max(
              this.inverseDeterminate *
                (-this.plane.y * spriteX_relativeToPlayer +
                  this.plane.x * spriteY_relativeToPlayer),
              0
            );

            const spriteScreenX = Math.floor(
              (this.game.screen.width / 2) * (1 + transformX / transformY)
            );
            ...
            const spriteWidth = Math.abs(
              Math.floor((this.game.screen.height / transformY) * widthRatio)
            );
      ...
      if (
        closestUnobstructedSpriteWithTrigger.sprite &&
        closestUnobstructedSpriteWithTrigger.sprite.trigger != null
      ) {
