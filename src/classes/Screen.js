...
const getWallCellTextureCode = (cell, wallFace) => {
  if (
    wallFace &&
    typeof cell.faces === "object" &&
    cell.faces[wallFace] != null &&
    typeof cell.faces[wallFace] === "object"
  ) {
...
const getFaceSpecificWallConfig = (cell, wallFace) => {
  ...
  if (cell.faces != null) {
    ...
    if (
      typeof faceSpecificConfig === "object" &&
      faceSpecificConfig != null &&
      faceSpecificConfig.hasOwnProperty("textureType") &&
      faceSpecificConfig.hasOwnProperty("textureConfig")
    ) {
...
const getWallColor = (wallTextureConfig, brightness) => {
  ...
  switch (colorType) {
    case "hex":
      ...
      color = hasLeadingHash
        ? wallTextureConfig.color
        : "#" + wallTextureConfig.color;
      break;
    case "hsl":
      const defaultHSL = {
...
const validateWallTextureConfig = (textureType, textureConfig) => {
  ...
  if (textureType === "color") {
    const hasColor =
      textureConfig.hasOwnProperty("colorType") &&
      VALID_COLOR_TYPES.indexOf(textureConfig.colorType) > -1;
    if (hasColor) {
      // TODO: Validate color.
...
class Screen {
  ...
  generateSkyGradient(stops) {
    ...
    const gradient = this.staticPOVBackgroundCtxBuffer.createLinearGradient(
      0,
      0,
      0,
      this.height / 2
    ...
    this.staticPOVBackgroundCtxBuffer.fillRect(
      0,
      0,
      this.width,
      this.height / 2
  ...
  initializeOffscreenCanvasPixels() {
    if (!this.offscreenCanvasPixels) {
      ...
      this.offscreenCanvasPixels = this.floorCtxBuffer.getImageData(
        0,
        0,
        this.width,
        this.height
  ...
  drawMapOverlay() {
    ...
    for (let rowOffset = 0; rowOffset < mapGrid.length; rowOffset++) {
      ...
      for (let columnOffset = 0; columnOffset < row.length; columnOffset++) {
        ...
        const cellTexture =
          this.game.images[cell - 1] && this.game.images[cell - 1].getCanvas();
        ...
        if (cellTexture) {
          ...
          this.ctxBuffer.beginPath();
          this.ctxBuffer.fillStyle = cellHue
            ? `hsla(${cellHue}, 100%, 80%, .9)`
            : emptyCellColor;
          this.ctxBuffer.fillRect(
            cellLeft,
            cellTop,
            mapWidthUnit,
            mapHeightUnit
  ...
  drawPOVBackground() {
    ...
    switch (textureType) {
      ...
      case "gradient":
        ...
        const fallbackGradientStops = [
          { stop: 0, color: "#6190E8" },
          { stop: 1, color: "#A7BFE8" },
        ];
        const gradientStops =
          stops && stops.length > 0 ? stops : fallbackGradientStops;
        if (!this.hasDrawnSkyGradient) {
          this.generateSkyGradient(gradientStops);
        }
        this.ctxBuffer.drawImage(
          this.staticPOVBackgroundCanvasBuffer,
          0,
          0,
          this.width,
          this.height / 2
  ...
  drawPlayerPOV() {
    ...
    for (let i = 0; i < rays.length; i++) {
      ...
      const isValidTextureConfig = validateWallTextureConfig(
        wallTextureType,
        wallTextureConfig
      );
      ...
      if (isValidTextureConfig) {
        ...
        // If it's an image or a gradient we will need a canvas to work from (gradients shouldn't be created on each pass, but cached).
          ...
          const wallTextureImageBuffer =
            wallTextureCode === "default"
              ? fallBackTexture_Rainbow
              : this.game.textureMap[wallTextureCode];
      ...
      if (floorColumnHeight > 0) {
        for (let y = columnBottom; y < this.height; y++) {
          ...
          const currentFloorX =
            weight * floorXWall + (1.0 - weight) * this.game.player.pos.x;
          const currentFloorY =
            weight * floorYWall + (1.0 - weight) * this.game.player.pos.y;

          const gridCell = this.currentMapGrid.getCell(
          ...
          if (gridCell !== null && gridCell !== undefined) {
            ...
            if (gridCell.textureType === "color") {
              ...
              this.offscreenCanvasPixels.data[index + 1] =
                g * brightnessModifier;
              this.offscreenCanvasPixels.data[index + 2] =
            ...
            const hasCeiling =
              gridCell.hasOwnProperty("ceilingConfig") &&
              gridCell.ceilingConfig != null;
            if (hasCeiling) {
              ...
              if (ceilingConfig.textureType === "color") {
                // TODO: This is going to be a bit of a slow down. We should require floors to be hex?
                const ceilingColor = getWallColor(
                  ceilingConfig.textureConfig,
                  brightness
                );
                // TODO: Here is a big breaking assumption. ALL floor colors are hex!
                ...
                this.offscreenCanvasPixels.data[index] =
                ...
                if (ceilingTexture != null) {
                  ...
                  const ceilTexY =
                    Math.floor(
                      (this.height - currentFloorY) * ceilingTexture.height
    ...
    this.floorCtxBuffer.clearRect(
      0,
      0,
      this.floorBuffer.width,
      this.floorBuffer.height
    );
    this.offscreenCanvasPixels = this.floorCtxBuffer.getImageData(
      0,
      0,
    ...
    if (sprites.length > 0) {
      ...
      const sortedSprites = sprites.sort((sprite1, sprite2) => {
        const sprite1distance =
          Math.pow(this.game.player.pos.x - sprite1.pos.x, 2) +
          Math.pow(this.game.player.pos.y - sprite1.pos.y, 2);
        const sprite2distance =
          Math.pow(this.game.player.pos.x - sprite2.pos.x, 2) +
          Math.pow(this.game.player.pos.y - sprite2.pos.y, 2);
      ...
      for (let i = 0; i < sortedSprites.length; i++) {
        ...
        if (currentSprite) {
          ...
          const transformX =
            this.game.player.inverseDeterminate *
            (this.game.player.dir.x * spriteY_relativeToPlayer -
              this.game.player.dir.y * spriteX_relativeToPlayer);
          ...
          const transformY = Math.max(
            this.game.player.inverseDeterminate *
              (-this.game.player.plane.y * spriteX_relativeToPlayer +
                this.game.player.plane.x * spriteY_relativeToPlayer),
            0
          );
          ...
          const spriteHeight = Math.abs(
            Math.floor(this.height / transformY) * scale
          );

          // calculate lowest and highest pixel to fill in current stripe
          const drawStartY = Math.floor(
            (-spriteHeight * currentSprite.verticalOffset) / 2 + this.height / 2
          );
          ...
          const widthRatio =
            (currentSprite.width / currentSprite.height) * scale;
          const spriteWidth = Math.abs(
            Math.floor((this.height / transformY) * widthRatio)
          );
